...
b <- c(effect, 0)
c <- rep(effect, 2)
# error scales in mediation model with normal errors
sigma_m <- sqrt(1 - a^2)
...
results_list <- lapply(sample_sizes, function(n) {
  ...
  results_n <- lapply(1:K, function(k) {
    ...
    x <- rnorm(n)
    e_m <- rlnorm(n) - exp(1/2)
    e_y <- rlnorm(n) - exp(1/2)
...
## store results
file <- "simulations/results/results_lognormal.RData"
