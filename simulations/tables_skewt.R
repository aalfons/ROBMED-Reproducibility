...
parameters <- c("n", "alpha", "nu", "Skewness", "Kurtosis")
methods <- c("ols_boot", "ols_sobel", "bcn_boot",
             "huber_boot", "median_boot", "robust_boot")

## nice labels to be used in tables
parameter_labels <- c(n = "n", alpha = "gamma", nu = "nu",
                      Skewness = "Skewness", Kurtosis = "Excess kurtosis")
...
file_results <- "simulations/results/results_skewt.RData"
load(file_results)
...
## compute skewness and excess kurtosis for parameter scenarios
df_parameter <- results %>% distinct(alpha, nu)
gammas <- mapply(function(alpha, nu) {
  if (is.infinite(nu)) nu <- 1e10
  tryCatch(unname(dp2cp(c(0, 1, alpha, nu), family = "ST"))[3:4],
           warning = function(condition) rep(NA_real_, 2))
}, alpha = df_parameter$alpha, nu = df_parameter$nu)
df_parameter <- cbind(df_parameter, Skewness = gammas[1, ],
                      Kurtosis = gammas[2, ])
...
for (i in 1:nrow(scenarios)) {
  ...
  df_ab <- df_ab %>%
    ...
    left_join(df_parameter, by = c("alpha", "nu")) %>%
    arrange(n, desc(nu), alpha)
  ...
  df_reject[, 2] <- round(df_reject[, 2], 3)
  df_reject[, -(1:3)] <- lapply(df_reject[, -(1:3)],
                                function(x) sprintf("%.3f", x))

  # file name for .csv file with table
  if (scenarios$b[i] == 0) {
    file_table <- "simulations/tables/table_skewt_a=%.1f_b=%d_%s.csv"
  } else {
    file_table <- "simulations/tables/table_skewt_a=%.1f_b=%.1f_%s.csv"
  }
