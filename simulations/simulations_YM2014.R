...
sample_sizes <- c(50, 100, 200, 500)  # number of observations
effects <- c(0.14, 0.39, 0.59)        # true effects
a <- c(effects, rep(0, length(effects)))
b <- rep(effects, 2)
c <- rep(effects, 2)
seed <- 20150326                      # seed for the random number generator
K <- 1000                             # number of simulation runs
R <- 5000                             # number of bootstrap samples
settings <- c("normal", "t", "mixture")
...
generate_errors <- function(n, df = 2, epsilon = 0.1) {
  # generate standard normal errors
  e_normal <- rnorm(n)
  # generate t distributed errors: first apply the standard normal CDF to
  # transform to uniformly distributed values, then apply the inverse CDF
  # of the t distribution
  u <- pnorm(e_normal)
  e_t <- qt(u, df = df)
  # generate mixture of normal errors
  sigma <- ifelse(runif(n) < 1 - epsilon, 1, 10)
  e_mixture <- sigma * e_normal
  # combine the error terms as matrix
  cbind(normal = e_normal, t = e_t, mixture = e_mixture)
}
...
results_list <- lapply(sample_sizes, function(n) {
  ...
  results_n <- lapply(1:K, function(k) {
    ...
    ## (for maximum comparability of the results, the different error
    ## terms are transformed from the normal errors generated above)
    results_nk <- lapply(settings, function(setting) {

      ## loop over different parameter settings
      results_nke <- mapply(function(a, b, c) {

        # ## print simulation run
        # cat(paste(Sys.time(), sprintf(":      ab = %.04f\n", a * b)))

        # generate hypothesized mediator and dependent variable
        m <- a * x + e_m[, setting]
        y <- b * m + c * x + e_y[, setting]
...
file <- "simulations/results/results_YM2014.RData"
