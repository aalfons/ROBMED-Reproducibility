...
results <- results %>%
  filter(Method %in% methods, d == 0 | epsilon == 0.02) %>%
...
for (i in 1:nrow(scenarios)) {
  ...
  df_ylim <- data.frame(Parameter = factor(facet_labels[parameters],
                                           levels = facet_labels),
                        d = 0, Value = c(ab_ylim, reject_ylim))

  # plot results
  mapping <- aes(x = d, y = Value, color = Method,
                 linetype = Method, size = Method)
  p <- ggplot() +
    geom_blank(mapping = aes(x = d, y = Value), data = df_ylim) +
  ...
  # save plot to file
  if (scenarios$b[i] == 0) {
    file_plot <- "simulations/figures/figure_outliers_a=%.1f_b=%d.pdf"
  } else {
    file_plot <- "simulations/figures/figure_outliers_a=%.1f_b=%.1f.pdf"
  }
