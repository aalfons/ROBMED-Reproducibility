...
results <- results %>%
  ...
  filter(Method %in% methods, Shift == 0 | Epsilon == 0.02)
...
for (i in 1:nrow(scenarios)) {
  ...
  df_ylim <- data.frame(Parameter = factor(facet_labels[parameters],
                                           levels = facet_labels),
                        Shift = 0, Value = c(ab_ylim, reject_ylim))

  # plot results
  mapping <- aes(x = Shift, y = Value, color = Method,
                 linetype = Method, size = Method)
  p <- ggplot() +
    geom_blank(mapping = aes(x = Shift, y = Value), data = df_ylim) +
    geom_hline(aes(yintercept = Value), data = df_true) +
    geom_line(mapping = mapping, data = df_results) +
    facet_grid(Parameter ~ n, scales = "free", labeller = "label_parsed") +
    labs(x = "Shift of outliers", y = NULL) +
  ...
  # save plot to file
  if (scenarios$b[i] == 0) {
    file_plot <- "simulations/figures/figure_ZY2010-shift_a=%.1f_b=%d.pdf"
  } else {
    file_plot <- "simulations/figures/figure_ZY2010-shift_a=%.1f_b=%.1f.pdf"
  }
