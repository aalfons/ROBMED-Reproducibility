...
file_results <- "simulations/results/results_Fleishman.RData"
load(file_results)
...
for (i in 1:nrow(scenarios)) {
  ...
  df_ab <- results_ab %>%
    group_by(n, Skewness, Kurtosis, Method) %>%
  ...
  if (scenarios$b[i] == 0) {
    file_table <- "simulations/tables/table_Fleishman_a=%.1f_b=%d_%s.csv"
  } else {
    file_table <- "simulations/tables/table_Fleishman_a=%.1f_b=%.1f_%s.csv"
  }
