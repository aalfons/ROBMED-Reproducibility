...
results_list <- lapply(1:K, function(k) {
  ...
  results_k <- lapply(settings, function(setting) {

    ## print simulation run
    cat(paste(Sys.time(), sprintf(":    setting = \"%s\"\n", setting)))

    ## loop over different parameter values
    results_ks <- mcmapply(function(a, b, c, sigma_m, sigma_y) {
      ...
      df_ols <- tryCatch({
        ...
        data.frame(Run = k, Setting = setting, a = a, b = b,
    ...
    SIMPLIFY = FALSE, mc.set.seed = FALSE, mc.cores = 2)

    ## 'mc.set.seed = FALSE' is set such that seed of the random number
    ## generator is passed on from main thread to the worker threads.  The
    ## only place inside the parallelized loop where random numbers are need
    ## is the subsampling algorithm of the MM estimator.  By setting this
    ## option, the same subsamples are used across parameter settings.  Note
    ## that the variability in the samples is guaranteed by performing the
    ## random draws outside the parallelized loop.

    ## combine results for current simulation run into data frame
    do.call(rbind, results_ks)
...
file <- "simulations/results/results_ORM.RData"
save(results, n, a, b, c, level, session_info, file = file)
